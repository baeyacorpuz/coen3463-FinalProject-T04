    ...
    fetch('api/v1/department?sort=createdate').then(function(res) {
      res.json().then(function(departments) {
        ...
        departments.forEach(function(department) {
          tbody.insertAdjacentHTML('beforeend', '<li><a class="panel-block is-active" href="/departments/' + department._id + '">' + department.comp_name + '<span class="panel-icon"><i class="fa fa-book"</span></a></li>');
        });
      })
    });
    ...
    fetch('api/v1/department/count').then(function(res) {
      res.json().then(function(departments) { 
        ...
        var count = document.getElementById('count');
          count.insertAdjacentHTML('beforeend', '<h1>Total Number of Components: '+departments.count+'</h1>');
      
      })
    });
  }
  else{
    fetch('api/v1/department?query={"comp_name":"~(' + localStorage.getItem("search") + ')"}').then(function(res) {
      res.json().then(function(departments){
